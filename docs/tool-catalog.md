...
| `@deepseek-ai/dsh-tool-computer-use` | `computer_use` | `ctx.tools`, `ctx.computerUse (execution time)`, `ctx.attachments`, `ctx.systemPrompt` | `tool/call`, `tool/result` | - | computer_use drives the shared surface through ctx.computerUse and attaches each observation as a durable image, so the schema stays stable across provider swaps. |
...
Drive the controllable browser surface. Every action returns the post-action screenshot (as an image), the page URL, and the page title.
